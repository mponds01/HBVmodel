...
@everywhere function characteristics_d()
        ...
        Yearly_temperatures = Float64[]
        Yearly_potential_evaporation = Float64[]
        Yearly_discharge = Float64[]
        Yearly_precipitation = Float64[]
        #create yearly averages of each timeseries
        startdate = 1986
        enddate = 2004

        for t in startdate:enddate
                T_yearstart = findfirst(isequal(Date(t, 1, 1)), Dates_Temperature_Daily)
                T_yearend = findfirst(isequal(Date(t, 12, 31)), Dates_Temperature_Daily)
