...
function budyko_plot(startyear,endyear)#All_Catchment_Names, Area_Catchments)
    ...
    #Plots.savefig("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Projections/Budyko/Past/All_catchments_all_data_loss2.png")
    ...
    Plots.savefig("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Projections/Budyko/Results/Budyko_Parameterized.png")

    #Plot for only hargreaves
    p_hg = Plots.plot(title = "Budyko space hg", titlefontsize=12)
    plot!(Epot_Prec, Budyko_Eact_P, label="Original Budyko", linecolor="black")
    for (i, catchment) in enumerate(All_Catchments)
        if catchment != "Gailtal"
            plot!(Epot_Prec, Budyko_eact_P_all_hg[:,i], label=catchment, linecolor=Color[i], linestyle=:solid)#, title="Catchment specific locations using Hargreaves Ep")
            scatter!([AI_all_hg[i]], [EI_all[i]], label=catchment, color=[Color[i]], markershape=[Markers[4]], markersize=3, markerstrokewidth=0)
        end
    ...
    #Plots.savefig("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Projections/Budyko/Past/All_catchments_hg_loss2.png")

    p_tw = Plots.plot(title = "Budyko space tw original", titlefontsize=12)
    plot!(Epot_Prec, Budyko_Eact_P, label="Original Budyko", linecolor="black")
    plot!(collect(0:5),collect(0:5), linestyle=:dot, linecolor="black", label="Energy Limit")#, size=(2200,1200))
    plot!(collect(1:5), ones(5), linestyle=:dot, linecolor="black", label="Water Limit")
    for (i, catchment) in enumerate(All_Catchments)
            plot!(Epot_Prec, Budyko_eact_P_all_tw[:,i], label=catchment, linecolor=Color[i], linestyle=:dot)#, title="Catchment specific locations using Thornthwaite Ep" )
            scatter!([AI_all_tw[i]], [EI_all[i]], label=catchment, color=[Color[i]], markershape=[Markers[3]], markersize=3, markerstrokewidth=0)
            xaxis!("Ep/P")
            yaxis!("Ea/P")
            #vline!([0.406])
            xlims!((0,2))
    ...
    Plots.savefig("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Projections/Budyko/Past/All_catchments_tw_loss2.png")



    CSV.write("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Projections/Budyko/Past/All_catchments_omega_tw_loss.csv", Catchment_data_tw)
    CSV.write("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Projections/Budyko/Past/All_catchments_omega_hg_loss.csv", Catchment_data_hg)
    CSV.write("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Projections/Budyko/Past/All_catchments_omega_all_loss.csv", Catchment_data_all)
    CSV.write("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Projections/Budyko/Past/All_catchments_observed_meandata_loss.csv", Catchment_observed_data_all)
    return #Catchment_observed_data_all
end

budyko_plot(1981,2010)
...
function Pitztal_correction_plot()
    Color=["blue", "lightblue"]
    EI_all = Float64[]
    labels = ["Corrected", "Original"]
    AI_tw, AI_hg, EI_c, P_observed_mean, Epot_observed_mean_tw, Epot_observed_mean_hg = aridity_evaporative_index_Pitztal_loss(1983,2005)
    AI_tw, AI_hg, EI, P_observed_mean, Epot_observed_mean_tw, Epot_observed_mean_hg = aridity_evaporative_index_Pitztal(1983,2005)
    push!(EI_all, EI_c, EI)

    budyko_pitztal = Plots.plot(title="Correction Pitztal", fontsize=12, figsize=(12,10))

    Plots.plot!(collect(0:5),collect(0:5), linestyle=:dot, linecolor="black", label="Energy Limit", size=(700,470))
    Plots.plot!(collect(1:5), ones(5), linestyle=:dot, linecolor="black", label="Water Limit")
    Epot_Prec = collect(0:0.1:5)
    w = 2.65
    Budyko_Eact_P_fu = (ones(length(Epot_Prec))) + Epot_Prec .* ones(length(Epot_Prec)) - ((ones(length(Epot_Prec)))+ Epot_Prec.^w).^(1/w)
    Budyko_Eact_P = ( Epot_Prec .* tanh.(1 ./Epot_Prec) .* (ones(length(Epot_Prec)) - exp.(-Epot_Prec))).^0.5
    ...
    plot!(Epot_Prec, Budyko_Eact_P_fu, label="Budyko curve", linecolor="black", legend=:bottomright)
    println(AI_tw)
    AI_tw_p = AI_tw .* ones(2)

    for i in 1:2
        scatter!([AI_tw_p[i]], [EI_all[i]], label=labels[i], color=[Color[i]], markershape=:circle, markersize=3, markerstrokewidth=0)
    end
    ...
    xlims!(0,2)
    ylims!(0.2,1)
    display(budyko_pitztal)
    Plots.savefig("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Projections/Budyko/Past/correction_Pitztal.png")
end

#Pitztal_correction_plot()
