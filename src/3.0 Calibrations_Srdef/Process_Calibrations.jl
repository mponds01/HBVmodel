...
function getbest_parametersets_ns_srdef(path_to_file, path_to_folder, number_best)
    calibration = readdlm(path_to_file, ',')
    # sort the calibration according to the euclidean distance
...
function getbest_parametersets_ns_all()
    catchments = ["Defreggental", "Feistritz", "Gailtal", "Palten", "Pitztal", "Silbertal"]
    times = ["MF", "OP"]
    for (c,cm) in enumerate(catchments)
        path = "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/"*cm*"/Samples/"
            ...
            for (r,rcp) in enumerate(rcps)
                    ...
                    for (j,rcm) in enumerate(rcms)
                        ...
                        println(cm, rcp, rcm)
                        path_to_save = "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/"*cm*"/Best_Samples/"*tf*"/"
                        getbest_parametersets_ns_srdef(path*rcp*"/NS_Parameterset_"*tf*"_"*rcp*"_"*rcm*".csv", path_to_save)
                    end
                end
            end
            return
...
function combine_calibrations_srdef(path, path_to_save)
    ...
    writedlm(path_to_save*"NS_Parameterfit_srdef_calibrated_combined.csv", all_calibrations, ',')
end


function combine_all_calibrations_srdef()
        ...
        for (c, cm) in enumerate(catchment)
                ...
                #rcms = readdir(local_path*rcp)
                        ...
                        for (j,rcm) in enumerate(rcms)
                            ...
                            println("current loop:", cm, ", ", rcp,  ", ", rcm)
                            path_to_calibrations = "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/"*cm*"/Calibrated/"*rcp*"/"*rcm*"/"
                            combine_calibrations_srdef(path_to_calibrations, path_to_calibrations)
                        end
                    end
                end
                return
            end
combine_all_calibrations_srdef()
...
"""
function calibration_statistics(catchment, number_best, lower_boundary_y_axis)
    path_to_file =
    path_to_save =
    max_Obj = Float64[]
    max_NSE = Float64[]
