...
@everywhere function run_MC_pi(ID, rcp, rcm)
        ...
        count = 1
        number_Files = 0
        parameters_best_calibrations = readdlm("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/Pitztal/Samples/"*rcp*"/NS_Parameterset_OP_"*rcp*"_"*rcm*".csv", ',')
        ...
        for n in 1 : 1:size(parameters_best_calibrations)[1]
                ...
                if Goodness_Fit != -9999
                        ...
                        if size(All_Goodness)[2]-1 == 100
                                ...
                                if count != 100
                                        open(local_path*"Calibrations_Srdef/Pitztal/Calibrated/"*rcp*"/"*rcm*"/NS_Parameterfit_srdef_calibrated_"*string(ID)*"_"*string(number_Files)*".csv", "a") do io
        ...
        All_Goodness = transpose(All_Goodness[:, 2:end])
        open(local_path*"Calibrations_Srdef/Pitztal/Calibrated/"*rcp*"/"*rcm*"/NS_Parameterfit_srdef_calibrated"*string(ID)*".csv", "a") do io
                writedlm(io, All_Goodness,",")
        end
end
...
# @time begin
# run_MC_pi(1,500)
# pmap(ID -> run_MC_pi(ID, nmax) , [1])
# end

function runMC_pitztal()
        ...
        #rcms = readdir(local_path*rcp)
                ...
                for (j,rcm) in enumerate(rcms)
                        ...
                        #transorm collection ID using function run_MC_pi, whereas process is distributed over all computer workers except for 1
                        pmap(ID -> run_MC_pi(ID, rcp, rcm) , [1])
                        end
                end
        end
        return
