        ...
        for i in 1: length(ID_Prec_Zones)
                ...
                for (h, current_elevation) in enumerate(Elevation_Zone_Catchment)
                        ...
                        if counter <= elevations && Elevations_Each_Precipitation_Zone[i][counter] == current_elevation
                                ...
                                #savefig("Gailtal/Calibration_8.05/Plots/Snowstorage_"*string(current_elevation)*string(ID_Prec_Zones[i])*"_large.png")
...
function run_bestparameters_pitztal(path_to_best_parameter, nmax, startyear, endyear)
        ...
        Temperature_17315 = CSV.read(local_path*"HBVModel/Pitztal/prenner_tag_17315.dat", DataFrame, header = true, skipto = 3, delim = ' ', ignorerepeated = true)
        Temperature_17315 = dropmissing(Temperature_17315)
        Temperature_Array_17315= Temperature_17315.t / 10
        Timeseries_Temp_17315 = Date.(Temperature_17315.datum, Dates.DateFormat("yyyymmdd"))
        startindex_17315 = findfirst(isequal(Date(2008, 1, 1)), Timeseries_Temp_17315)
        endindex_17315 = findfirst(isequal(Date(endyear, 12, 31)), Timeseries_Temp_17315)
        Temperature_Daily_17315 = Temperature_Array[startindex_17315[1]:endindex_17315[1]]

        Dates_Temperature_Daily_17315 = Timeseries_Temp_17315[startindex_17315[1]:endindex_17315[1]]
        # # append!(Temperature_Daily_all, Temperature_Array_13120[index_missing_dataset] + ones(length(index_missing_dataset))*0.6)
        # # append!(Temperature_Daily_all, Temperature_Daily[index+1:end])
        Dates_Temperature_Daily_all = Array{Date,1}(undef, 0)
        #Temperature_Daily_all = Array{Float64,1}(undef, 0)
        # # index where Dates are missing
        append!(Dates_Temperature_Daily_all, Dates_Temperature_Daily)
        append!(Dates_Temperature_Daily_all, Dates_Temperature_Daily_17315)
        # append!(Dates_Temperature_Daily_all, Dates_Temperature_Daily[index+1:end])
        Temperature_Daily_all = Array{Float64,1}(undef, 0)

        append!(Temperature_Daily_all, Temperature_Daily)
        append!(Temperature_Daily_all, Temperature_Daily_17315)# =+ ones(length(index_missing_dataset))*0.6)
        # append!(Temperature_Daily_all, Temperature_Daily[index+1:end])
        #Temperature_Daily = Temperature_Daily_all
        @assert Dates_Temperature_Daily_all == Timeseries

        Elevation_Zone_Catchment, Temperature_Elevation_Catchment, Total_Elevationbands_Catchment = gettemperatureatelevation(Elevations_Catchment, Temperature_Daily_all)
        # get the temperature data at the mean elevation to calculate the mean potential evaporation
        Temperature_Mean_Elevation = Temperature_Elevation_Catchment[:,findfirst(x-> x==Mean_Elevation_Catchment, Elevation_Zone_Catchment)]
        Potential_Evaporation = getEpot_Daily_thornthwaite(Temperature_Mean_Elevation, Timeseries, Sunhours_Vienna)

         # index = findall(x -> x == Date(2007,31,12) - Day(1), Dates_Temperature_Daily)[1]
         # index_17315 = findall(x -> x == Date(2007,31,12) - Day(1), Dates_Temperature_Daily)[1]
        ...
        best_calibrations = readdlm(path_to_best_parameter, ',')
        println(size(best_calibrations))

        parameters_best_calibrations = best_calibrations[1:nmax,10:30]
        println(size(parameters_best_calibrations))

        loss_term=true
        #All_discharge = Array{Any, 1}[]
        for n in 1 : 1:size(parameters_best_calibrations)[1]
                #print(typeof(all_inputs))
                loss_term=parameters_best_calibrations[n,end]
...
function run_bestparameters_silbertal(path_to_best_parameter, nmax, startyear, endyear)
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
...
function run_bestparameters_montafon_ohnegletscher(path_to_best_parameter, nmax, startyear, endyear, scale_factor_Discharge)
        ...
        Elevations_16910 = Elevations(200, 800, 2990, 1028, 1140)
        Elevations_100057 = Elevations(200, 1600, 3400, 2000, 1140)
        Elevations_100180 = Elevations(200, 400, 2990, 681, 1140)
        Elevations_100206 = Elevations(200, 600, 2800, 897, 1140)
...
function run_bestparameters_gailtal_NS(path_to_best_parameter, nmax, startyear, endyear)
        ...
        best_calibrations = readdlm(path_to_best_parameter, ',')
        parameters_best_calibrations = best_calibrations[1:nmax,1:end]
...
function run_bestparameters_palten_NS(path_to_best_parameter, nmax, startyear, endyear)
...
function run_bestparameters_feistritz_NS(path_to_best_parameter, nmax, startyear, endyear)
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        ID_Prec_Zones = [109967]
        # size of the area of precipitation zones
...
function run_bestparameters_pitztal_NS(path_to_best_parameter, nmax, startyear, endyear)
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        # 102061 im Norden
        ID_Prec_Zones = [102061, 102046]
...
function run_bestparameters_silbertal_NS(path_to_best_parameter, nmax, startyear, endyear)
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        scale_factor_Discharge = 0.65
...
function run_bestparameters_montafon_ohnegletscher_NS(path_to_best_parameter, nmax, startyear, endyear, scale_factor_Discharge)
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        ID_Prec_Zones = [100123, 16910, 100057, 100180, 100206]
        # size of the area of precipitation zones
...
function run_bestparameters_montafon_mitgletscher_NS(path_to_best_parameter, nmax, startyear, endyear)
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        ID_Prec_Zones = [100123, 16910, 100057, 100180, 100206]
        # size of the area of precipitation zones
...
function run_bestparameters_defreggental_NS(path_to_best_parameter, nmax, startyear, endyear)
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        ID_Prec_Zones = [17700, 114926]
        # size of the area of precipitation zones
...
# All_Discharges, All_GWstorage, ALl_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation = run_bestparameters_gailtal("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Gailtal/Parameterfit_less_dates_snow_redistr_1M_best_300.csv"", 298, 1983, 2013)
# Catchment_Name = "Paltental"
# Area_Zones =
Area_Catchment_Palten = sum([198175943.0, 56544073.0, 115284451.3])
# All_Discharges, All_GWstorage, ALl_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation = run_bestparameters_palten("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Paltental/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 300, 1983, 2013)
# Catchment_Name = "Feistritz"
# Area_Zones =
Area_Catchment_Feistritz = sum([115496400.])
#All_Discharges, All_GWstorage, ALl_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation = run_bestparameters_feistritz("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Feistritz/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 300, 1983, 2015)
# Catchment_Name = "Palten"
# writedlm("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Projections/"*Catchment_Name*"/Comparison_Real_Proj/Discharge/Discharge_obs_85_13.csv", Observed_Discharge)
# writedlm("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Projections/"*Catchment_Name*"/Comparison_Real_Proj/Discharge/Discharge_mod__measured_data_85_13.csv", All_Discharges)
# Catchment_Name = "Pitztal_loss_less"
Area_Catchment_Pitztal =  sum([20651736.0, 145191864.0])
#All_Discharges, All_GWstorage, ALl_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation, Potential_Evaporation  = run_bestparameters_pitztal("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 300, 1983, 2007, 1)
#Catchment_Name ="Pitztal"

# Catchment_Name = "Silbertal"
# Area_Zones =
Area_Catchment_Silbertal = sum([100139168.])
# All_Discharges, All_GWstorage, ALl_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation = run_bestparameters_silbertal("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Silbertal/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 300, 1983, 2013, 0.7)

Catchment_Name = "Defreggental"
# Area_Zones =
println("loading all discharges")
# Area_Catchment_Defreggental = sum([235811198.0, 31497403.0])
# All_Discharges, All_GWstorage, All_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation = run_bestparameters_defreggental("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Defreggental/Best/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 300,1983, 2015)
# All_Discharges_NS, All_GWstorage_NS, All_Snowstorage_NS, All_Snow_Elevations_NS, All_Soilstorage_NS, All_Snow_Cover_Modeled_NS, All_Snow_Cover_Observed_NS, Observed_Discharge_NS, Timeseries_NS, All_Faststorage_NS, Total_Precipitation_NS, Temperature_Mean_Elevation_NS = run_bestparameters_defreggental_NS("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/Silbertal/Samples/rcp45/NS_Parameterset_MF_rcp45_CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day.csv", 300,1983, 2015)

# Catchment_Name = "Montafon"
# Area_Catchment_Montafon = sum([119964251.0, 140378202.0, 71358347.0, 135149166.0, 66631637.0])
# All_Discharges, All_GWstorage, ALl_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation = run_bestparameters_montafon_mitgletscher("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Silbertal/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 300, 1985, 2007)
...
function plot_hydrographs(All_Discharges, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Catchment_Name, Nr_Years, Area_Catchment, nr_runs)
        for i in 1:Nr_Years
                ...
                if current_year == 1990
                        ...
                        xlabel!("Time in Year")
                        plot!(Timeseries[indexfirstday:indexlasttday], convertDischarge(Observed_Discharge[indexfirstday:indexlasttday], Area_Catchment), label="Observed",size=(1800,1000), color = ["darkorange"], linewidth = 3)
                        ...
                        hline!([0], color="black")
                        temperature = plot!()
                        plot(discharge, precipitation, temperature, layout= (3,1), legend = false, size=(2990,1800), left_margin = [5mm 0mm], bottom_margin = 20px, xrotation = 60, yguidefontsize=20, xtickfont = font(20), ytickfont = font(20), dpi=300)
                        savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/Discharges/Discharge_All_"*string(current_year)*"_with_Prec_large.png")
                        #savefig("Discharge_All_"*string(current_year)*"_with_Prec_other_Temp_font_large.png")
                        #savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/Discharges/Discharge_All_"*string(current_year)*"_with_Prec_other_Temp_font_large.png")
                end
        end
end
...
function plot_hydrographs_final_report(All_Discharges, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Catchment_Name, Nr_Years, Area_Catchment, nr_runs)
        ...
        for i in [5,25]#1:Nr_Years
                ...
                plot!(Timeseries[indexfirstday:indexlasttday], convertDischarge(Observed_Discharge[indexfirstday:indexlasttday], Area_Catchment), label="Measured Runoff",size=(1800,1000), color = ["darkorange"], linewidth = 3, framestyle = :box, gridalpha = 0.3)
                ...
                Plots.savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/Discharges/Discharge_"*string(current_year)*"_final_report_large.png")
        end
end

function plot_hydrographs_final_report_total(All_Discharges, All_Discharges_NS, Observed_Discharge, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Timeseries_NS, Catchment_Name, Nr_Years, Area_Catchment, nr_runs)
        # get mean of 300 parameter simulations
        println("converting S discharges for plotting")
        println(size(All_Discharges))
        mean_discharge = mean(All_Discharges, dims = 2)
        max_discharge = maximum(All_Discharges, dims = 2)
        min_discharge = minimum(All_Discharges, dims= 2)
        #convert discharge to mm/d
        ...
        max_discharge = convertDischarge(max_discharge, Area_Catchment)
        min_discharge = convertDischarge(min_discharge, Area_Catchment)
        println("converting NS discharges for plotting")
        # All_Discharges_NS = readdlm("/Volumes/Magali 2/Data_projected/Past/NS/"*Catchment_Name*"/rcp45/CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_SMHI-RCA4_v1_day/combined_NS_streamflow_projection_Past.csv",  ',')
        # println(size(All_Discharges_NS))
        # All_Discharges_NS=transpose(All_Discharges_NS)
        println(size(All_Discharges_NS))


        mean_discharge_NS = mean(All_Discharges_NS, dims = 2)
        max_discharge_NS = maximum(All_Discharges_NS, dims = 2)
        min_discharge_NS = minimum(All_Discharges_NS, dims= 2)
        #convert discharge to mm/d
        mean_discharge_NS = convertDischarge(mean_discharge_NS, Area_Catchment)
        max_discharge_NS = convertDischarge(max_discharge_NS, Area_Catchment)
        min_discharge_NS = convertDischarge(min_discharge_NS, Area_Catchment)
        #println(size(mean_discharge))
        for i in [5,25]#1:Nr_Years
                current_year = 1985+i
                if current_year != 1986 && current_year != 1985 + Nr_Years
                        indexfirstday = findall(x -> x == Dates.firstdayofyear(Date(current_year,1,1)), Timeseries)[1]
                        indexlasttday = findall(x -> x == Dates.lastdayofyear(Date(current_year,1,1)), Timeseries)[1]
                        indexfirstday_NS = findall(x -> x == Dates.firstdayofyear(Date(current_year,1,1)), Timeseries_NS)[1]
                        indexlasttday_NS = findall(x -> x == Dates.lastdayofyear(Date(current_year,1,1)), Timeseries_NS)[1]
                ...
                elseif current_year == 1985 + Nr_Years
                        ...
                        indexlasttday = length(Timeseries)
                        indexfirstday_NS = findall(x -> x == Dates.firstdayofyear(Date(current_year,1,1)), Timeseries_NS)[1]
                        indexlasttday_NS = length(Timeseries_NS)

                end

                Plots.plot(xaxis=nothing)
                plot!(Timeseries[indexfirstday:indexlasttday], mean_discharge[indexfirstday:indexlasttday], ribbon = (mean_discharge[indexfirstday:indexlasttday] - min_discharge[indexfirstday:indexlasttday], max_discharge[indexfirstday:indexlasttday] - mean_discharge[indexfirstday:indexlasttday]), color = ["grey"], size=(1800,1000), linewidth=3, fillalpha=.5, label="Modelled S", xaxis=nothing)
                plot!(Timeseries_NS[indexfirstday_NS:indexlasttday_NS], mean_discharge_NS[indexfirstday_NS:indexlasttday_NS], ribbon = (mean_discharge_NS[indexfirstday_NS:indexlasttday_NS] - min_discharge_NS[indexfirstday_NS:indexlasttday_NS], max_discharge_NS[indexfirstday_NS:indexlasttday_NS] - mean_discharge_NS[indexfirstday_NS:indexlasttday_NS]), color = ["darkorange"], label="Modelled NS", size=(1800,1000), linewidth=3, fillalpha=.5)
                ylabel!("Discharge [mm/d]")
                title!(Catchment_Name*" "*string(current_year), titlefont=font(22))
                plot!(Timeseries[indexfirstday:indexlasttday], convertDischarge(Observed_Discharge[indexfirstday:indexlasttday], Area_Catchment), label="Observed",size=(1800,1000), color = ["black"], linewidth = 3, framestyle = :box, gridalpha = 0.3, xaxis=false)
                xlims!((Dates.value(Timeseries[indexfirstday]), Dates.value(Timeseries[indexlasttday])))
                discharge = plot!()
                #plot!(p, Timeseries[indexfirstday:indexlasttday], Total_Precipitation[indexfirstday:indexlasttday], label="Observed Rainfall",size=(1800,1000), color = ["blue"], linewidth = 3)
                Plots.plot(Timeseries[indexfirstday:indexlasttday], Total_Precipitation[indexfirstday:indexlasttday], label=false,size=(1800,1000), color = ["blue"], linewidth = 2, framestyle = :box, gridalpha = 0.3, xaxis=nothing)
                ylabel!("Precipitation [mm/d]")
                xlims!((Dates.value(Timeseries[indexfirstday]), Dates.value(Timeseries[indexlasttday])))
                precipitation = plot!()
                # plot Temperature
                Plots.plot(Timeseries[indexfirstday:indexlasttday], Temperature_Mean_Elevation[indexfirstday:indexlasttday], label=false,size=(1800,1000), color = ["red"], linewidth = 2, framestyle = :box, gridalpha = 0.3)
                ylabel!("Temperature [°C]")
                xlims!((Dates.value(Timeseries[indexfirstday]), Dates.value(Timeseries[indexlasttday])))
                temperature = plot!()
                if i == 1
                        tot = plot(discharge, precipitation, temperature, layout= (3,1), legend = true, size=(2500,1800), left_margin = [10mm 0mm], bottom_margin = 25px,  top_margin = 15px, xrotation = 0, yguidefontsize=20, xtickfont = font(20), ytickfont = font(20), dpi=300, legendfont=font(16))
                else
                        tot = plot(discharge, precipitation, temperature, layout= (3,1), legend = true, size=(2500,1800), left_margin = [10mm 0mm], bottom_margin = 25px, top_margin = 15px, xrotation = 0, yguidefontsize=20, xtickfont = font(20), ytickfont = font(20), dpi=300, legendfont=font(16))
                end
                #display(tot)
                Plots.savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/Discharges/Discharge_"*string(current_year)*"_final_report_NS_large.png")
                Plots.savefig("/Volumes/Magali 2/Results/Calibrations/Discharges/"*Catchment_Name*"_Discharge_"*string(current_year)*"_final_report_NS_large.png")

        end
end
function plot_hydrographs_final_report_combined_input(All_Discharges, All_Discharges_NS, Observed_Discharge, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Timeseries_NS, Catchment_Name, Nr_Years, Area_Catchment, nr_runs)
        # get mean of 300 parameter simulations
        println("converting S discharges for plotting")
        println(size(All_Discharges))
        mean_discharge = mean(All_Discharges, dims = 2)
        ...
        for i in [5,25]#1:Nr_Years
                ...
                # xtixks = Array([indexfirstday+100, indexfirstday+200, indexfirstday+300])#: Dates.value(Timeseries[indexlasttday])),3)
                if i==5 && Catchment_Name == "Feistritz"
                        println("in loop")
                        Plots.plot()
                        plot!(Timeseries_NS[indexfirstday_NS:indexlasttday_NS], mean_discharge_NS[indexfirstday_NS:indexlasttday_NS], ribbon = (mean_discharge_NS[indexfirstday_NS:indexlasttday_NS] - min_discharge_NS[indexfirstday_NS:indexlasttday_NS], max_discharge_NS[indexfirstday_NS:indexlasttday_NS] - mean_discharge_NS[indexfirstday_NS:indexlasttday_NS]), color = ["darkorange"], label="Modelled NS", linewidth=3, fillalpha=.5)

                        plot!(Timeseries[indexfirstday:indexlasttday], mean_discharge[indexfirstday:indexlasttday], ribbon = (mean_discharge[indexfirstday:indexlasttday] - min_discharge[indexfirstday:indexlasttday], max_discharge[indexfirstday:indexlasttday] - mean_discharge[indexfirstday:indexlasttday]), color = ["grey"],  linewidth=3, fillalpha=.5, label="Modelled S")
                        # ylabel!("Discharge [mm/d]")
                        title!(Catchment_Name*" "*string(current_year), titlefont=font(20))
                        plot!(Timeseries[indexfirstday:indexlasttday], convertDischarge(Observed_Discharge[indexfirstday:indexlasttday], Area_Catchment), label="Observed", color = ["black"], linewidth = 3, framestyle = :box, gridalpha = 0.3, xaxis=false)
                        xlims!((Dates.value(Timeseries[indexfirstday]), Dates.value(Timeseries[indexlasttday])))
                        discharge = plot!()
                else
                        Plots.plot()
                        plot!(Timeseries_NS[indexfirstday_NS:indexlasttday_NS], mean_discharge_NS[indexfirstday_NS:indexlasttday_NS], ribbon = (mean_discharge_NS[indexfirstday_NS:indexlasttday_NS] - min_discharge_NS[indexfirstday_NS:indexlasttday_NS], max_discharge_NS[indexfirstday_NS:indexlasttday_NS] - mean_discharge_NS[indexfirstday_NS:indexlasttday_NS]), color = ["darkorange"], label=false, linewidth=3, fillalpha=.5)

                        plot!(Timeseries[indexfirstday:indexlasttday], mean_discharge[indexfirstday:indexlasttday], ribbon = (mean_discharge[indexfirstday:indexlasttday] - min_discharge[indexfirstday:indexlasttday], max_discharge[indexfirstday:indexlasttday] - mean_discharge[indexfirstday:indexlasttday]), color = ["grey"],  linewidth=3, fillalpha=.5, label=false)
                        if Catchment_Name == "Defreggental"
                                ylabel!("Discharge [mm/d]")
                                plot!()
                        end
                        #Array(((Dates.value(Timeseries[indexfirstday]), Dates.value(Timeseries[indexlasttday]))))
                        title!(Catchment_Name*" "*string(current_year), titlefont=font(20))
                        plot!(Timeseries[indexfirstday:indexlasttday], convertDischarge(Observed_Discharge[indexfirstday:indexlasttday], Area_Catchment), label=false, color = ["black"], linewidth = 3, framestyle = :box, gridalpha = 0.3, xaxis=false)
                        xlims!((Dates.value(Timeseries[indexfirstday]), Dates.value(Timeseries[indexlasttday])))
                        xticks!([Dates.value(Timeseries[indexfirstday+100]),Dates.value(Timeseries[indexfirstday+200]),Dates.value(Timeseries[indexfirstday+300])], ["", "", ""])

                        if i==25
                                xlabel!("Time [calendary day]", )
                                # println(indexfirstday+100)
                                xticks!([Dates.value(Timeseries[indexfirstday+100]),Dates.value(Timeseries[indexfirstday+200]),Dates.value(Timeseries[indexfirstday+300])], ["100", "200", "300"])
                        end
                ...
                runoff_plot = Plots.plot(discharge,  legend=true, size=(3400,900), left_margin = [20mm 0mm], bottom_margin = 50px,  top_margin = 25px,  xrotation = 0, yguidefontsize=20, xguidefontsize=20,xtickfont = font(20), ytickfont = font(20), dpi=300, legendfont=font(20))

                cm = push!(cm, runoff_plot)

                # Plots.savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/Discharges/Discharge_"*string(current_year)*"_final_report_NS_large.png")
                # Plots.savefig("/Volumes/Magali 2/Results/Calibrations/Discharges/"*Catchment_Name*"_Discharge_"*string(current_year)*"_final_report_NS_large.png")
        end
        total = Plots.plot(cm[1], cm[2], layout=grid(2,1), size=(3400,1800), xaxis=true, legend=true)
        display(total)
        return total
end

function combined_hydrograph_plot()
        plot=[]
        Catchment_Name = "Defreggental"
        Area_Catchment = Area_Catchment_Defreggental
                All_Discharges, All_GWstorage, All_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation = run_bestparameters_defreggental("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/"*Catchment_Name*"/Best/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 300,1983, 2015)
                All_Discharges_NS, All_GWstorage_NS, All_Snowstorage_NS, All_Snow_Elevations_NS, All_Soilstorage_NS, All_Snow_Cover_Modeled_NS, All_Snow_Cover_Observed_NS, Observed_Discharge_NS, Timeseries_NS, All_Faststorage_NS, Total_Precipitation_NS, Temperature_Mean_Elevation_NS = run_bestparameters_defreggental_NS("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/"*Catchment_Name*"/Samples/rcp45/NS_Parameterset_MF_rcp45_CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day.csv", 2500,1983, 2015)
                total_d = plot_hydrographs_final_report_combined_input(All_Discharges, All_Discharges_NS, Observed_Discharge, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Timeseries_NS, Catchment_Name, 27, Area_Catchment, 300)
        Catchment_Name = "Feistritz"
        Area_Catchment = Area_Catchment_Feistritz
                All_Discharges, All_GWstorage, All_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation = run_bestparameters_feistritz("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/"*Catchment_Name*"/Best/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 300,1983, 2015)
                All_Discharges_NS, All_GWstorage_NS, All_Snowstorage_NS, All_Snow_Elevations_NS, All_Soilstorage_NS, All_Snow_Cover_Modeled_NS, All_Snow_Cover_Observed_NS, Observed_Discharge_NS, Timeseries_NS, All_Faststorage_NS, Total_Precipitation_NS, Temperature_Mean_Elevation_NS = run_bestparameters_feistritz_NS("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/"*Catchment_Name*"/Samples/rcp45/NS_Parameterset_MF_rcp45_CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day.csv", 2500,1983, 2015)
                total_f = plot_hydrographs_final_report_combined_input(All_Discharges, All_Discharges_NS, Observed_Discharge, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Timeseries_NS, Catchment_Name, 27, Area_Catchment, 300)
        combined = Plots.plot(total_d, total_f, layout=grid(1,2), size=(2500,1200), grid=false, yguidefontsize=font(30), xguidefontsize=font(20), legendfontsize=20)
        display(combined)
        # Plots.savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/Discharge_2_CM_years_compared_large.png")
        # Plots.savefig("/Volumes/Magali 2/Results/Calibrations/Discharges/Discharge_2_CM_years_compared_large.png")

        return
end

combined_hydrograph_plot()

Catchment_Names=["Defreggental", "Feistritz", "Gailtal", "Pitztal", "Palten", "Silbertal"]
Catchment_Areas = [Area_Catchment_Defreggental, Area_Catchment_Feistritz, Area_Catchment_Gailtal, Area_Catchment_Pitztal, Area_Catchment_Palten, Area_Catchment_Silbertal]


# plot_hydrographs_final_report(All_Discharges, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Catchment_Name, 27, Area_Catchment_Defreggental, 300)
# for (c,Catchment_Name) in enumerate(Catchment_Names)
#         Area_Catchment = Catchment_Areas[c]
#         if Catchment_Name=="Defreggental"
#                 All_Discharges, All_GWstorage, All_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation = run_bestparameters_defreggental("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/"*Catchment_Name*"/Best/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 300,1983, 2015)
#                 All_Discharges_NS, All_GWstorage_NS, All_Snowstorage_NS, All_Snow_Elevations_NS, All_Soilstorage_NS, All_Snow_Cover_Modeled_NS, All_Snow_Cover_Observed_NS, Observed_Discharge_NS, Timeseries_NS, All_Faststorage_NS, Total_Precipitation_NS, Temperature_Mean_Elevation_NS = run_bestparameters_defreggental_NS("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/"*Catchment_Name*"/Samples/rcp45/NS_Parameterset_MF_rcp45_CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day.csv", 2990,1983, 2015)
#                 plot_hydrographs_final_report_total(All_Discharges, All_Discharges_NS, Observed_Discharge, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Timeseries_NS, Catchment_Name, 27, Area_Catchment, 300)
#
...
#                 All_Discharges, All_GWstorage, All_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation = run_bestparameters_gailtal("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/"*Catchment_Name*"/Best/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 3,1983, 2012)
#                 All_Discharges_NS, All_GWstorage_NS, All_Snowstorage_NS, All_Snow_Elevations_NS, All_Soilstorage_NS, All_Snow_Cover_Modeled_NS, All_Snow_Cover_Observed_NS, Observed_Discharge_NS, Timeseries_NS, All_Faststorage_NS, Total_Precipitation_NS, Temperature_Mean_Elevation_NS = run_bestparameters_gailtal_NS("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/"*Catchment_Name*"/Samples/rcp45/NS_Parameterset_MF_rcp45_CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day.csv", 2990,1983, 2012)
#                 plot_hydrographs_final_report_total(All_Discharges, All_Discharges_NS, Observed_Discharge, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Timeseries_NS, Catchment_Name, 27, Area_Catchment, 300)
#
#         elseif Catchment_Name=="Palten"
#                 All_Discharges, All_GWstorage, All_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation = run_bestparameters_palten("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/"*Catchment_Name*"/Best/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 3,1983, 2013)
#                 All_Discharges_NS, All_GWstorage_NS, All_Snowstorage_NS, All_Snow_Elevations_NS, All_Soilstorage_NS, All_Snow_Cover_Modeled_NS, All_Snow_Cover_Observed_NS, Observed_Discharge_NS, Timeseries_NS, All_Faststorage_NS, Total_Precipitation_NS, Temperature_Mean_Elevation_NS = run_bestparameters_palten_NS("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/"*Catchment_Name*"/Samples/rcp45/NS_Parameterset_MF_rcp45_CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day.csv", 2990,1983, 2013)
#                 plot_hydrographs_final_report_total(All_Discharges, All_Discharges_NS, Observed_Discharge, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Timeseries_NS, Catchment_Name, 27, Area_Catchment, 300)
#
#         elseif Catchment_Name=="Pitztal"
#                 All_Discharges, All_GWstorage, All_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation = run_bestparameters_pitztal("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/"*Catchment_Name*"/Best/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 300,1983, 2015)
#                 All_Discharges_NS, All_GWstorage_NS, All_Snowstorage_NS, All_Snow_Elevations_NS, All_Soilstorage_NS, All_Snow_Cover_Modeled_NS, All_Snow_Cover_Observed_NS, Observed_Discharge_NS, Timeseries_NS, All_Faststorage_NS, Total_Precipitation_NS, Temperature_Mean_Elevation_NS = run_bestparameters_pitztal_NS("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/"*Catchment_Name*"/Samples/rcp45/NS_Parameterset_MF_rcp45_CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day.csv", 2990,1983, 2015)
#                 plot_hydrographs_final_report_total(All_Discharges, All_Discharges_NS, Observed_Discharge, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Timeseries_NS, Catchment_Name, 27, Area_Catchment, 300)
#
#         elseif Catchment_Name=="Silbertal"
#                 All_Discharges, All_GWstorage, All_Snowstorage, All_Snow_Elevations, All_Soilstorage, All_Snow_Cover_Modeled, All_Snow_Cover_Observed, Observed_Discharge, Timeseries, All_Faststorage, Total_Precipitation, Temperature_Mean_Elevation = run_bestparameters_silbertal("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/"*Catchment_Name*"/Best/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 300,1983, 2015)
#                 All_Discharges_NS, All_GWstorage_NS, All_Snowstorage_NS, All_Snow_Elevations_NS, All_Soilstorage_NS, All_Snow_Cover_Modeled_NS, All_Snow_Cover_Observed_NS, Observed_Discharge_NS, Timeseries_NS, All_Faststorage_NS, Total_Precipitation_NS, Temperature_Mean_Elevation_NS = run_bestparameters_silbertal_NS("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/"*Catchment_Name*"/Samples/rcp45/NS_Parameterset_MF_rcp45_CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day.csv", 2990,1983, 2015)
#                 plot_hydrographs_final_report_total(All_Discharges, All_Discharges_NS, Observed_Discharge, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Timeseries_NS, Catchment_Name, 27, Area_Catchment, 300)
#
#         end
# end
...
function plot_gwstorage(GW_Storage, Timeseries, Catchment_Name, Nr_Years, nr_runs)
        for i in 1:Nr_Years
                ...
                #plot!(Timeseries[indexfirstday:indexlasttday], Observed_Discharge[indexfirstday:indexlasttday], label="Observed",size=(1800,1000), color = ["darkorange"])
                ylabel!("Slow Storage [mm]")
                xlabel!("Time in Year")
                savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/GWStorage/GW_Storage_"*string(current_year)*"_large.png")
        end
        ...
        ylabel!("Slow Storage [mm]")
        xlabel!("Time in Year")
        savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/GWStorage/GW_Storage_all_years"*string(current_year)*"_large.png")
        plot()
        for h in 1:nr_runs
                mean_GW = Float64[]
                mean_Date = Date[]
                ...
                plot!(mean_Date, mean_GW, color = ["darkorange"], legend=false, size=(1800,1000))

        end
        hline!([200, 400], color=("black"))
        ylabel!("Slow Storage [mm]")
        xlabel!("Time in Year")
        savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/GWStorage/GW_Storage_mean_all_years_large.png")
        #plot!(Timeseries[indexfirstday:indexlasttday], Observed_Discharge[indexfirstday:indexlasttday], label="Observed",size=(1800,1000), color = ["darkorange"])
end
...
function plot_snowstorage(Snowstorage, Timeseries, Catchment_Name, Nr_Years, nr_runs)
        for i in 1:Nr_Years
                ...
                xlabel!("Time in Year")
                savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/Snow_Storage/Snow_Storage_"*string(current_year)*"_large.png")
        end
        ...
        ylabel!("Snow Storage [mm]")
        xlabel!("Time in Year")
        savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/Snow_Storage/Snow_Storage_all_years"*string(current_year)*"_large.png")
end
...
function plot_snowcover(All_Snow_Cover_Modeled, All_Snow_Cover_Observed, min_elevation, max_elevation, Catchment_Name, Timeseries)
        ...
        for i in 1:1
                ...
                savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/Snow_Cover/Snow_Cover_high_elevations2_large.png")
...
function plot_soilstorage(Soilstorage, Timeseries, Catchment_Name)
        ...
        for h in 1:20
                ...
                savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/Soil_Storage/Soilstorage_best_"*string(h)*"_large.png")
        end
end
...
function plot_faststorage(Faststorage, Timeseries, Catchment_Name)
        ...
        for h in 1:20
                ...
                title!("Best parameter set: "*Catchment_Name)
                savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/Fast_Storage/Faststorage_best_"*string(h)*"_large.png")
        end
end
...
function plot_snow_elevation(All_Snow_Elevations, Timeseries, min_elevation, max_elevation, Catchment_Name)
        ...
        #ylims!((0,1000))
        savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"/Snow_Storage_Elevation/201_300_bestruns_large.png")
end
...
#         display(modelled)
#         display(observed)
#         #plot_mo = Plots.plot([modelled, observed])
#         #display(plot_mo)
# end
...
# #xlims!((0,1))
# ylims!((0,1))
# savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Projections/Pitztal/"*Catchment_Name*"_budykoframework_past_real_Data_mean_large.png")


# function plot_hydrographs_mean(All_Discharges, Total_Precipitation, Temperature_Mean_Elevation, Timeseries, Catchment_Name, Nr_Years, Area_Catchment, nr_runs)
#         for i in 1:Nr_Years
#                 current_year = 1985+i
#                 if current_year != 1986 && current_year != 1985 + Nr_Years
#                         indexfirstday = findall(x -> x == Dates.firstdayofyear(Date(current_year,1,1)), Timeseries)[1]
...
#                 discharges=zeros()
#                 for h in 1:nr_runs
#                         discharge_day =  convertDischarge(All_Discharges[:, h], Area_Catchment))
#                         discharges = hcat(discharges, discharge_day)
#                         if h==nr_runs
#                                 prinln(size(discharges))
#                         end
#                         # plot!(Timeseries[indexfirstday:indexlasttday], convertDischarge(All_Discharges[indexfirstday:indexlasttday, h], Area_Catchment), color = ["black"], legend=false, size=(1800,1000))
#
#                 end
...
#                         plot!(Timeseries[indexfirstday:indexlasttday], convertDischarge(Observed_Discharge[indexfirstday:indexlasttday], Area_Catchment), label="Observed",size=(1800,1000), color = ["darkorange"], linewidth = 3)
#                         discharge = plot!()#p = twinx()
#                         savefig("/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Calibration/"*Catchment_Name*"_Discharge_All_"*string(current_year)*".png")
#                 end
#         end
# end
