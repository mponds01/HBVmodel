...
@everywhere function run_MC_pa(ID, rcp, rcm)
        ...
        count = 1
        number_Files = 0
        parameters_best_calibrations = readdlm("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/Palten/Samples/"*rcp*"/NS_Parameterset_OP_"*rcp*"_"*rcm*".csv", ',')

        parameters_best_calibrations = parameters_best_calibrations[1:3000,:]

        #parameters_best_calibrations = parameters_best_calibrations[2001:end,:]
        ...
        for n in 1 : 1:size(parameters_best_calibrations)[1]
                ...
                # println(n)#+2600)
                # println(parameters_best_calibrations[n, :])
                ...
                # parameters, slow_parameters, parameters_array = parameter_selection_palten_srdef(min_srdef_Grass, min_srdef_Forest, min_srdef_Bare, min_srdef_Rip, max_srdef_Grass, max_srdef_Forest, max_srdef_Bare, max_srdef_Rip)
                ...
                if Goodness_Fit != -9999
                        ...
                        if size(All_Goodness)[2]-1 == 100
                                ...
                                if count != 100
                                        open(local_path*"Calibrations_Srdef/Palten/Calibrated/"*rcp*"/"*rcm*"/NS_Parameterfit_srdef_calibrated_"*string(ID)*"_"*string(number_Files)*".csv", "a") do io
        ...
        All_Goodness = transpose(All_Goodness[:, 2:end])
        open(local_path*"Calibrations_Srdef/Palten/Calibrated/"*rcp*"/"*rcm*"/NS_Parameterfit_srdef_calibrated"*string(ID)*".csv", "a") do io
                writedlm(io, All_Goodness,",")
        end
end
#
...
# @time begin
# #run_MC_pa(1,500)
# pmap(ID -> run_MC_pa(ID, nmax) , [1,2])
# end

function runMC_palten()
        rcps=["rcp45", "rcp85"]#["rcp45", "rcp85"]
        for (i, rcp) in enumerate(rcps)
        #rcms = readdir(local_path*rcp)
                #rcms = ["CNRM-CERFACS-CNRM-CM5_"*rcp*"_r1i1p1_CLMcom-CCLM4-8-17_v1_day"]
                rcms=["CNRM-CERFACS-CNRM-CM5_"*rcp*"_r1i1p1_CNRM-ALADIN53_v1_day", "CNRM-CERFACS-CNRM-CM5_"*rcp*"_r1i1p1_SMHI-RCA4_v1_day", "ICHEC-EC-EARTH_"*rcp*"_r1i1p1_KNMI-RACMO22E_v1_day", "ICHEC-EC-EARTH_"*rcp*"_r3i1p1_DMI-HIRHAM5_v1_day","ICHEC-EC-EARTH_"*rcp*"_r3i1p1_DMI-HIRHAM5_v1_day",
                ...
                for (j,rcm) in enumerate(rcms)
                        ...
                        #transorm collection ID using function run_MC_pa, whereas process is distributed over all computer workers except for 1
                        pmap(ID -> run_MC_pa(ID, rcp, rcm) , [1])
                        end
                end
        end
        return
