...
@everywhere function run_MC_g(ID, rcp,rcm)
        ...
        count = 1
        number_Files = 0
        parameters_best_calibrations = readdlm("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/Gailtal/Samples/"*rcp*"/NS_Parameterset_OP_"*rcp*"_"*rcm*".csv", ',')
        ...
        for n in 1 : 1:size(parameters_best_calibrations)[1]
                ...
                # parameters, slow_parameters, parameters_array = parameter_selection_gailtal_srdef(min_srdef_Grass, min_srdef_Forest, min_srdef_Bare, min_srdef_Rip, max_srdef_Grass, max_srdef_Forest, max_srdef_Bare, max_srdef_Rip)
                # parameter ranges
                ...
                if Goodness_Fit != -9999
                        ...
                        if size(All_Goodness)[2]-1 == 100
                                ...
                                if count != 100
                                        open(local_path*"Calibrations_Srdef/Gailtal/Calibrated/"*rcp*"/"*rcm*"/NS_Parameterfit_srdef_calibrated_"*string(ID)*"_"*string(number_Files)*".csv", "a") do io
        ...
        All_Goodness = transpose(All_Goodness[:, 2:end])
        open(local_path*"Calibrations_Srdef/Gailtal/Calibrated/"*rcp*"/"*rcm*"/NS_Parameterfit_srdef_calibrated"*string(ID)*".csv", "a") do io
                writedlm(io, All_Goodness,",")
        end
...
function runMC_gailtal()
        ...
        #rcms = readdir(local_path*rcp)
                ...
                for (j,rcm) in enumerate(rcms)
                        ...
                        #transorm collection ID using function run_MC_g, whereas process is distributed over all computer workers except for 1
                        pmap(ID -> run_MC_g(ID, rcp, rcm) , [1])
                        end
                end
        end
        return
