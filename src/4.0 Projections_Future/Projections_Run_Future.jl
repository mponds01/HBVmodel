...
function run_projections_gailtal(path_to_projection, path_to_best_parameter, startyear, endyear, spinup, period, sns, rcp, rcm, cm)
        ...
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        ...
        parameters_best_calibrations = parameters_best_calibrations[1:300,:]
        ...
        path_result = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Data_Projected/"*period*"/"*sns*"/"*cm*"/"*rcp*"/"*rcm*"/"
        ...
        writedlm(path_result*sns*"_model_results_discharge_"*period*".csv", All_Discharge, ',')
        writedlm(path_result*sns*"_model_results_snow_storage_"*period*".csv", All_Snowstorage, ',')
        writedlm(path_result*sns*"_model_results_GW_storage_"*period*".csv", All_GWstorage, ',')
...
function run_projections_paltental(path_to_projection, path_to_best_parameter, startyear, endyear, spinup, period, sns, rcp, rcm, cm)
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        ...
        writedlm(path_result*sns*"_model_results_snow_storage_"*period*".csv", All_Snowstorage, ',')
        writedlm(path_result*sns*"_model_results_GW_storage_"*period*".csv", All_GWstorage, ',')
        #writedlm(path_result*sns*"_model_results_snow_cover_"*period*".csv", All_Snow_Cover, ',')
        #writedlm(path_to_projection*"results_epot_"*period*".csv", Potential_Evaporation[index_spinup: index_lastdate], ',')
        #writedlm(path_to_projection*"results_precipitation_"*period*".csv", Total_Precipitation[index_spinup: index_lastdate], ',')
        return #All_Discharge
end
...
function run_projections_defreggental(path_to_projection, path_to_best_parameter, startyear, endyear, spinup, period, sns, rcp, rcm, cm)
        ...
        #save the results for the projections
        #writedlm(path_to_projection*"100_model_results_05_10.csv", All_Goodness, ',')
        writedlm(path_result*sns*"_model_results_discharge_snow_redistr_"*period*".csv", All_Discharge, ',')
        writedlm(path_result*sns*"_model_results_snow_storage_"*period*".csv", All_Snowstorage, ',')
        writedlm(path_result*sns*"_model_results_GW_storage_"*period*".csv", All_GWstorage, ',')
...
function run_projections_silbertal(path_to_projection, path_to_best_parameter, startyear, endyear, spinup, period, sns, rcp, rcm, cm)
        ...
        for n in 1 : 1:size(parameters_best_calibrations)[1]
                ...
                Discharge, GWstorage, Snow_Storage = runmodelprecipitationzones_future(Potential_Evaporation, Precipitation_All_Zones, Temperature_Elevation_Catchment, Current_Inputs_All_Zones, Current_Storages_All_Zones, Current_GWStorage, parameters, slow_parameters, Area_Zones, Area_Zones_Percent, Elevation_Percentage, Elevation_Zone_Catchment, ID_Prec_Zones, Nr_Elevationbands_All_Zones, Elevations_Each_Precipitation_Zone)
                All_Discharge = hcat(All_Discharge, Discharge[index_spinup:index_lastdate])
                All_Snowstorage = hcat(All_Snowstorage, Snowstorage[index_spinup:index_lastdate])
                All_GWstorage = hcat(All_GWstorage, GWstorage[index_spinup: index_lastdate])
                #Discharge, GWstorage, Snowstorage = runmodelprecipitationzones_all_output(Potential_Evaporation, Precipitation_All_Zones, Temperature_Elevation_Catchment, Current_Inputs_All_Zones, Current_Storages_All_Zones, Current_GWStorage, parameters, slow_parameters, Area_Zones, Area_Zones_Percent, Elevation_Percentage, Elevation_Zone_Catchment, ID_Prec_Zones, Nr_Elevationbands_All_Zones, Elevations_Each_Precipitation_Zone)
...
function run_projections_pitztal(path_to_projection, path_to_best_parameter, startyear, endyear, spinup, period, sns, rcp, rcm, cm)
        ...
        for n in 1 : 1:size(parameters_best_calibrations)[1]
                ...
                #All_GWstorage = hcat(All_GWstorage, GWstorage[index_spinup: index_lastdate])
                #All_Snowstorage = hcat(All_Snowstorage, Snowstorage[index_spinup: index_lastdate])
                Discharge, GWstorage, Snow_Melt  = runmodelprecipitationzones_glacier_future(Potential_Evaporation, Glacier_All_Zones, Precipitation_All_Zones, Temperature_Elevation_Catchment, Current_Inputs_All_Zones, Current_Storages_All_Zones, Current_GWStorage, parameters, slow_parameters, Area_Zones, Area_Zones_Percent, Elevation_Percentage, Elevation_Zone_Catchment, ID_Prec_Zones, Nr_Elevationbands_All_Zones, Elevations_Each_Precipitation_Zone)
                #calculate snow for each precipitation zone
                # don't calculate the goodness of fit for the spinup time!
        ...
        #save the results for the projections
        #writedlm(path_to_projection*"100_model_results_05_10.csv", All_Goodness, ',')
        writedlm(path_result*sns*"_model_results_discharge_snow_redistr_"*period*"_without_loss.csv", All_Discharge, ',')
...
function run_projections_feistritz(path_to_projection, path_to_best_parameter, startyear, endyear, spinup, period, sns, rcp, rcm, cm)
        ...
        for n in 1 : 1:size(parameters_best_calibrations)[1]
                ...
                Discharge, GWstorage, Snowstorage = runmodelprecipitationzones_all_output_f(Potential_Evaporation, Precipitation_All_Zones, Temperature_Elevation_Catchment, Current_Inputs_All_Zones, Current_Storages_All_Zones, Current_GWStorage, parameters, slow_parameters, Area_Zones, Area_Zones_Percent, Elevation_Percentage, Elevation_Zone_Catchment, ID_Prec_Zones, Nr_Elevationbands_All_Zones, Elevations_Each_Precipitation_Zone)
...
function run_all_projections()
        catchments = [  "Feistritz"]#, "Gailtal","Palten", "Pitztal", "Silbertal", "Defreggental",]#
                ...
                for (r,rcp) in enumerate(rcps)
                        ...
                        for (j,rcm) in enumerate(rcms)
                                ...
                                if cm == "Feistritz"
                                        ...
                                        # Discharge_future_NS = run_projections_feistritz(path*"/Pitten/", cal_NS_MF, 2071, 2100, 10, "Future", "NS", rcp, rcm, cm)
                                elseif cm == "Silbertal"
                                        ...
                                        # Discharge_future_NS = run_projections_silbertal(path*"/IllSugadin/", cal_NS_MF, 2071, 2100, 10, "Future", "NS", rcp, rcm, cm)
                                elseif cm == "Defreggental"
                                        ...
                                        # Discharge_future_NS = run_projections_defreggental(path*"/Defreggental/", cal_NS_MF, 2071, 2100, 10, "Future", "NS", rcp, rcm, cm)
                                elseif cm == "Gailtal"
                                        ...
                                        # Discharge_future_NS = run_projections_gailtal(path*"/Gailtal/", cal_NS_MF, 2071, 2100, 10, "Future", "NS", rcp, rcm, cm)
                                elseif cm == "Palten"
                                        Discharge_present_S = run_projections_paltental(path*"/Palten/", cal_S, 1981, 2010, 3, "Past", "S",rcp, rcm, cm)
                                        Discharge_future_S = run_projections_paltental(path*"/Palten/", cal_S, 2071, 2100, 10, "Future", "S", rcp, rcm, cm)
                                        # Discharge_present_NS = run_projections_paltental(path*"/Palten/", cal_NS_OP, 1981, 2010, 3, "Past", "NS", rcp, rcm, cm)
                                        # Discharge_future_NS = run_projections_paltental(path*"/Palten/", cal_NS_MF, 2071, 2100, 10, "Future", "NS", rcp, rcm, cm)
                                elseif cm == "Pitztal"
                                        ...
                                        # Discharge_future_NS = run_projections_pitztal(path*"/Pitztal/", cal_NS_MF, 2071, 2100, 10, "Future", "NS", rcp, rcm, cm)
                                end
                        end
                end
        end
...
run_all_projections()
#run_all_projections_validation()
