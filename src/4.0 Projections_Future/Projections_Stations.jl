...
"""
function run_projections_validation_gailtal(path_to_projection, path_to_best_parameter, startyear, endyear, sns, rcp, rcm, cm)
        ...
        path_result = "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Data_Projected/Validation/"*sns*"/"*cm*"/"*rcp*"/"*rcm*"/"

        All_Goodness = transpose(All_Goodness[:, 2:end])
        #All_Discharge = transpose(All_Discharge[:, 2:end])
        # save the results for the projections
        writedlm(path_result*"Validation_298_model_results_"*string(startyear+3)*"_"*string(endyear)*"_new.csv", All_Goodness, ',')
        #writedlm(path_result*"Validation_100_model_results_05_10_discharge.csv", All_Discharge, ',')
        return All_Goodness
end

function run_projections_validation_palten(path_to_projection, path_to_best_parameter, startyear, endyear, sns, rcp, rcm, cm)
        local_path = "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        ...
        #All_Discharge = transpose(All_Discharge[:, 2:end])
        # save the results for the projections
        writedlm(path_result*"Validation_300_model_results_"*string(startyear+3)*"_"*string(endyear)*"_correct.csv", All_Goodness, ',')
        #writedlm(path_result*"Validation_100_model_results_05_10_discharge.csv", All_Discharge, ',')
        return All_Goodness
end

function run_projections_validation_feistritz(path_to_projection, path_to_best_parameter, startyear, endyear, sns, rcp, rcm, cm)
        local_path = "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        ...
        #All_Discharge = transpose(All_Discharge[:, 2:end])
        # save the results for the projections
        writedlm(path_result*"Validation_model_results_"*string(startyear+3)*"_"*string(endyear)*".csv", All_Goodness, ',')
        #writedlm(path_result*"Validation_100_model_results_05_10_discharge.csv", All_Discharge, ',')
        return All_Goodness
end

function run_projections_validation_defreggental(path_to_projection, path_to_best_parameter, startyear, endyear, sns, rcp, rcm, cm)
        local_path = "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        ...
        #All_Discharge = transpose(All_Discharge[:, 2:end])
        # save the results for the projections
        writedlm(path_result*"Validation_300_model_results_snow_redistr_"*string(startyear+3)*"_"*string(endyear)*".csv", All_Goodness, ',')
        #writedlm(path_result*"Validation_100_model_results_05_10_discharge.csv", All_Discharge, ',')
        return All_Goodness
end

function run_projections_validation_silbertal(path_to_projection, path_to_best_parameter, startyear, endyear, sns, rcp, rcm, cm)
        local_path = "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
...
function run_projections_validation_pitztal(path_to_projection, path_to_best_parameter, startyear, endyear, sns, rcp, rcm, cm)
        local_path = "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        # 102061 im Norden
        ...
        #All_Discharge = transpose(All_Discharge[:, 2:end])
        # save the results for the projections
        writedlm(path_result*"Validation__model_results_snow_redistr_"*string(startyear+3)*"_"*string(endyear)*".csv", All_Goodness, ',')
        #writedlm(path_result*"Validation_100_model_results_05_10_discharge.csv", All_Discharge, ',')
        return All_Goodness
end

function run_all_projections_validation()
        catchments = [ "Gailtal", "Palten", "Pitztal", "Silbertal"] #"Defreggental", "Feistritz",
        for (c,cm) in enumerate(catchments)
                ...
                for (r,rcp) in enumerate(rcps)
                        ...
                        for (j,rcm) in enumerate(rcms)
                                ...
                                if cm == "Feistritz"
                                        Discharge_present_S = run_projections_validation_feistritz(path*"/Pitten/", cal_S, 1983, 2005, "S",rcp, rcm, cm)
                                        #Discharge_future_S = run_projections_validation_feistritz(path*"/Pitten/", cal_S, 2071, 2100, "S", rcp, rcm, cm)
                                        #Discharge_present_NS = run_projections_validation_feistritz(path*"/Pitten/", cal_NS_OP, 1983, 2005, "NS",rcp, rcm, cm)
                                        #Discharge_future_NS = run_projections_validation_feistritz(path*"/Pitten/", cal_NS_MF, 2071, 2100, "NS", rcp, rcm, cm)
                                elseif cm == "Silbertal"
                                        Discharge_present_S = run_projections_validation_silbertal(path*"/IllSugadin/", cal_S, 1983, 2005, "S",rcp, rcm, cm)
                                        #Discharge_future_S = run_projections_validation_silbertal(path*"/IllSugadin/", cal_S, 2071, 2100, "S", rcp, rcm, cm)
                                        #Discharge_present_NS = run_projections_validation_silbertal(path*"/IllSugadin/", cal_NS_OP, 1983, 2005, "NS",rcp, rcm, cm)
                                        #Discharge_future_NS = run_projections_validation_silbertal(path*"/IllSugadin/", cal_NS_MF, 2071, 2100, "NS", rcp, rcm, cm)
                                elseif cm == "Defreggental"
                                        Discharge_present_S = run_projections_validation_defreggental(path*"/Defreggental/", cal_S, 1983, 2005, "S",rcp, rcm, cm)
                                        #Discharge_future_S = run_projections_validation_defreggental(path*"/Defreggental/", cal_S, 2071, 2100, "S", rcp, rcm, cm)
                                        #Discharge_present_NS = run_projections_validation_defreggental(path*"/Defreggental/", cal_NS_OP, 1983, 2005, "NS",rcp, rcm, cm)
                                        #Discharge_future_NS = run_projections_validation_defreggental(path*"/Defreggental/", cal_NS_MF, 2071, 2100, "NS", rcp, rcm, cm)
                                elseif cm == "Gailtal"
                                        Discharge_present_S = run_projections_validation_gailtal(path*"/Gailtal/", cal_S, 1983, 2005, "S",rcp, rcm, cm)
                                        #Discharge_future_S = run_projections_validation_gailtal(path*"/Gailtal/", cal_S, 2071, 2100, "S", rcp, rcm, cm)
                                        #Discharge_present_NS = run_projections_validation_gailtal(path*"/Gailtal/", cal_NS_OP, 1983, 2005, "NS",rcp, rcm, cm)
                                        #Discharge_future_NS = run_projections_validation_gailtal(path*"/Gailtal/", cal_NS_MF, 2071, 2100, "NS", rcp, rcm, cm)
                                elseif cm == "Palten"
                                        Discharge_present_S = run_projections_validation_palten(path*"/Palten/", cal_S, 1983, 2005, "S",rcp, rcm, cm)
                                        #Discharge_future_S = run_projections_validation_palten(path*"/Palten/", cal_S, 2071, 2100, "S", rcp, rcm, cm)
                                        #Discharge_present_NS = run_projections_validation_palten(path*"/Palten/", cal_NS_OP, 1983, 2005, "NS",rcp, rcm, cm)
                                        #Discharge_future_NS = run_projections_validation_palten(path*"/Palten/", cal_NS_MF, 2071, 2100, "NS", rcp, rcm, cm)
                                elseif cm == "Pitztal"
                                        Discharge_present_S = run_projections_validation_pitztal(path*"/Pitztal/", cal_S, 1983, 2005, "S",rcp, rcm, cm)
                                        #Discharge_future_S = run_projections_validation_pitztal(path*"/Pitztal/", cal_S, 2071, 2100, "S", rcp, rcm, cm)
                                        #Discharge_present_NS = run_projections_validation_pitztal(path*"/Pitztal/", cal_NS_OP, 1983, 2005, "NS",rcp, rcm, cm)
                                        #Discharge_future_NS = run_projections_validation_pitztal(path*"/Pitztal/", cal_NS_MF, 2071, 2100, "NS", rcp, rcm, cm)
                                end
                        end
                end
        end
...
# #         observed_monthly_runoff = run_projections_validation_gailtal(path*name*"/Gailtal/", "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Gailtal_less_dates/Gailtal_Parameterfit_All_less_dates_best_proj.csv", 1983, 2005)
# # end
#
# # ---------- for Palten
# # #
# for (i, name) in enumerate(Name_Projections)
#         println(i)
#         observed_monthly_runoff = run_projections_validation_silbertal(path*name*"/IllSugadin/", "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Silbertal_Snow_Redistribution/Silbertal_Parameterfit_All_runs_snow_redistr_best_300.csv", 1983, 2005)
# end
...
# for (i, name) in enumerate(Name_Projections)
#         println(i)
#                 observed_monthly_runoff = run_projections_validation_pitztal(path*name*"/Pitztal/", "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal_Snow_Redistribution/Pitztal_Parameterfit_All_runs_snow_redistr_best_300.csv", 1983, 2005)
# end

# for (i, name) in enumerate(Name_Projections)
#         observed_monthly_runoff = run_projections_validation_feistritz(path*name*"/Pitten/", "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Feistritz/Feistritz_Parameterfit_All_best_100.csv", 1983, 2005)
# end

# for (i, name) in enumerate(Name_Projections)
#         observed_monthly_runoff = run_projections_validation_defreggental(path*name*"/Defreggental/", "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Defreggental_Snow_Redistribution/Defreggental_Parameterfit_All_runs_snow_redistr_best_300.csv", 1983, 2005)
# end
# println("new")
# for (i, name) in enumerate(Name_Projections)
