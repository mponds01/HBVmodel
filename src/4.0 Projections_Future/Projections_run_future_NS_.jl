...
function runmodelprecipitationzones_all_output_f(Potential_Evaporation::Array{Float64,1}, Precipitation_All_Zones::Array{Array{Float64,2},1}, Temperature_Elevation_Catchment::Array{Float64,2}, Inputs_All_Zones::Array{Array{HRU_Input,1},1}, Storages_All_Zones::Array{Array{Storages,1},1}, SlowStorage::Float64, parameters::Array{Parameters,1}, slow_parameters::Slow_Paramters, Area_Zones::Array{Float64,1}, Area_Zones_Percent::Array{Float64,1},            Elevation_Percentage::Array{Array{Float64,1},1},
        Elevation_Zone_Catchment::Array{Float64,1}, ID_Prec_Zones::Array{Int64,1},
        Nr_Elevationbands_All_Zones::Array{Int64,1}, Elevations_Each_Precipitation_Zone)
        Total_Discharge = zeros(length(Precipitation_All_Zones[1][:,1]))
        Total_Snowstorage = zeros(length(Precipitation_All_Zones[1][:,1]))
        Total_GWstorage = zeros(length(Precipitation_All_Zones[1][:,1]))
        #Total_Snow_Elevations = Array{Float64,2}[]
...
function run_projections_gailtal(path_to_projection, path_to_best_parameter, startyear, endyear, spinup, period, sns, rcp, rcm, cm)
        ...
        parameters_best_calibrations = best_calibrations[:,1:end]
        println(size(parameters_best_calibrations))
        if size(parameters_best_calibrations)[1]>=3000
                parameters_best_calibrations = parameters_best_calibrations[1:3000,:]
        end

        parameters_best_calibrations = parameters_best_calibrations[(findall(x->x!=0.0, parameters_best_calibrations[:,16])),:]
        println(size(parameters_best_calibrations))
        #parameters_best_calibrations = parameters_best_calibrations[1:3,1:end]

        All_Snow_Cover = transpose(length(Elevation_Zone_Catchment))
        for n in 1 : 1:size(parameters_best_calibrations)[1]
                if mod(n,100)==0
                        println("nr of runs ", n )
                elseif mod(n-1,500)==0
                       #println(size(All_Snowmelt))
                end
                ...
                path_result = "/Volumes/Magali 2/Data_projected/"*period*"/"*sns*"/"*cm*"/"*rcp*"/"*rcm*"/"

                if mod(n,500)==0 || n==size(parameters_best_calibrations)[1]
                ...
                #         All_Snow_Cover = All_Snow_Cover[2:end,:]
                        All_GWstorage= transpose(All_GWstorage[:,2:end])

                        #save the results for the projections
                        #writedlm(path_to_projection*"100_model_results_05_10.csv", All_Goodness, ',')
                        open(path_result*"WB_S_model_results_discharge_"*period*"_runs"*string(n)*".csv", "a") do io
                                writedlm(io, All_Discharge, ",")
                        end
                        # # writedlm(path_result*sns*"_model_results_snow_storage_"*period*".csv", All_Snowstorage, ',')
                        # open(path_result*sns*"_model_results_snow_melt_"*period*"_runs"*string(n)*".csv", "a") do io
                        #         writedlm(io, All_Snowmelt, ",")
                        # end
                        # open(path_result*sns*"_model_results_snow_cover_"*period*"_runs"*string(n)*".csv", "a") do io
                        #         writedlm(io, All_Snow_Cover, ',')
                        # end
                        open(path_result*"WB_S_model_results_GW_"*period*"_runs"*string(n)*".csv", "a") do io
                                writedlm(io, All_GWstorage, ',')                        end
...
function run_projections_palten(path_to_projection, path_to_best_parameter, startyear, endyear, spinup, period, sns, rcp, rcm, cm)
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
...
function run_projections_defreggental(path_to_projection, path_to_best_parameter, startyear, endyear, spinup, period, sns, rcp, rcm, cm)
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        ...
        for n in 1 : 1: size(parameters_best_calibrations)[1]
                ...
                #         All_Snow_Cover = All_Snow_Cover[2:end,:]
                        All_GWstorage= transpose(All_GWstorage[:,2:end])
                        println(size(All_GWstorage))
                        open(path_result*"WB_PforF2_model_results_GW_"*period*"_runs"*string(n)*".csv", "a") do io
                                writedlm(io, All_GWstorage, ',')
                        end
                        println(size(All_Discharge))
                        open(path_result*"WB_PforF2_model_results_discharge_"*period*"_runs"*string(n)*".csv", "a") do io
                                writedlm(io, All_Discharge, ",")
                        end
                        ...
                        #save the results for the projections
                        #writedlm(path_to_projection*"100_model_results_05_10.csv", All_Goodness, ',')
                        # writedlm(path_result*sns*"_model_results_discharge_"*period*"_runs"*string(n)*".csv", All_Discharge, ',')
                        # # writedlm(path_result*sns*"_model_results_snow_storage_"*period*".csv", All_Snowstorage, ',')
                        # writedlm(path_result*sns*"_model_results_snow_melt_"*period*"_runs"*string(n)*".csv", All_Snowmelt, ',')
                        # writedlm(path_result*sns*"_model_results_snow_cover_"*period*"_runs"*string(n)*".csv", All_Snow_Cover, ',')
                        All_Discharge = zeros(length(Timeseries_Obj))
                        # All_Snowstorage = zeros(length(Timeseries_Obj))
                        # All_Snowmelt = zeros(length(Timeseries_Obj))
                        # All_Snow_Cover = transpose(length(Elevation_Zone_Catchment))
        ...
        #All_Goodness = transpose(All_Goodness[:, 2:end])
        #writedlm(path_result*sns*"_model_results_snow_cover_snow_redistr_"*period*".csv", All_Snow_Cover, ',')
        #writedlm(path_to_projection*"results_epot_"*period*".csv", Potential_Evaporation[index_spinup: index_lastdate], ',')
        #writedlm(path_to_projection*"results_precipitation_"*period*".csv", Total_Precipitation[index_spinup: index_lastdate], ',')
        return #All_Discharge
end

function run_projections_silbertal(path_to_projection, path_to_best_parameter, startyear, endyear, spinup, period, sns, rcp, rcm, cm)
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
...
function run_projections_pitztal(path_to_projection, path_to_best_parameter, startyear, endyear, spinup, period, sns, rcp, rcm, cm)
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        # 102061 im Norden
...
function run_projections_feistritz(path_to_projection, path_to_best_parameter, startyear, endyear, spinup, period, sns, rcp, rcm, cm)
        local_path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
...
function run_all_projections()
        catchments = [ "Defreggental"]# "Defreggental", "Feistritz", "Palten", "Gailtal","Defreggental", "Feistritz", "Gailtal", "Palten","Defreggental",  "Feistritz", "Gailtal",
        for (c,cm) in enumerate(catchments)
                rcps = ["rcp45", "rcp85"]
                for (r,rcp) in enumerate(rcps)

                        rcms = ["CNRM-CERFACS-CNRM-CM5_"*rcp*"_r1i1p1_CLMcom-CCLM4-8-17_v1_day", "CNRM-CERFACS-CNRM-CM5_"*rcp*"_r1i1p1_CNRM-ALADIN53_v1_day", "CNRM-CERFACS-CNRM-CM5_"*rcp*"_r1i1p1_SMHI-RCA4_v1_day", "ICHEC-EC-EARTH_"*rcp*"_r1i1p1_KNMI-RACMO22E_v1_day",
                         "ICHEC-EC-EARTH_"*rcp*"_r3i1p1_DMI-HIRHAM5_v1_day", "ICHEC-EC-EARTH_"*rcp*"_r12i1p1_CLMcom-CCLM4-8-17_v1_day", "ICHEC-EC-EARTH_"*rcp*"_r12i1p1_SMHI-RCA4_v1_day", "IPSL-IPSL-CM5A-MR_"*rcp*"_r1i1p1_IPSL-INERIS-WRF331F_v1_day", "IPSL-IPSL-CM5A-MR_"*rcp*"_r1i1p1_SMHI-RCA4_v1_day",
                          "MOHC-HadGEM2-ES_"*rcp*"_r1i1p1_CLMcom-CCLM4-8-17_v1_day", "MOHC-HadGEM2-ES_"*rcp*"_r1i1p1_KNMI-RACMO22E_v1_day", "MOHC-HadGEM2-ES_"*rcp*"_r1i1p1_SMHI-RCA4_v1_day", "MPI-M-MPI-ESM-LR_"*rcp*"_r1i1p1_CLMcom-CCLM4-8-17_v1_day", "MPI-M-MPI-ESM-LR_"*rcp*"_r1i1p1_SMHI-RCA4_v1_day"]
                        for (j,rcm) in enumerate(rcms)

                                path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Data/Projections/"*rcp*"/"*rcm*"/"
                                cal_S = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/"*cm*"/Best/Parameterfit_less_dates_snow_redistr_1M_best_300.csv"
                                cal_NS_MF = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/"*cm*"/Samples/"*rcp*"/NS_Parameterset_MF_"*rcp*"_"*rcm*".csv"
                                cal_NS_OP = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations_Srdef/"*cm*"/Samples/"*rcp*"/NS_Parameterset_OP_"*rcp*"_"*rcm*".csv"
                                if cm == "Feistritz"
                                        println(cm, rcp, rcm)
                                        # Discharge_present_S = run_projections_feistritz(path*"/Pitten/", cal_S, 1981,2010, 3, "Past", "S",rcp, rcm, cm)
                                        # Discharge_future_S = run_projections_feistritz(path*"/Pitten/", cal_S, 2071,2100,10, "Future", "S", rcp, rcm, cm)
                                        # Discharge_present_NS = run_projections_feistritz(path*"/Pitten/", cal_NS_OP, 1981,2010 , 3, "Past", "NS", rcp, rcm, cm)
                                        Discharge_future_NS = run_projections_feistritz(path*"/Pitten/", cal_NS_OP, 2071,2100,10, "Future", "NS", rcp, rcm, cm)
                                elseif cm == "Silbertal"
                                        println(cm, rcp, rcm)
                                        # Discharge_present_S = run_projections_silbertal(path*"/IllSugadin/", cal_S, 1981,2010, 3, "Past", "S",rcp, rcm, cm)
                                        # Discharge_future_S = run_projections_silbertal(path*"/IllSugadin/", cal_S, 2071,2100,10, "Future", "S", rcp, rcm, cm)
                                        #Discharge_present_NS = run_projections_silbertal(path*"/IllSugadin/", cal_NS_OP, 1981,2010, 3, "Past", "NS", rcp, rcm, cm)
                                        Discharge_future_NS = run_projections_silbertal(path*"/IllSugadin/", cal_NS_OP, 2071,2100,10, "Future", "NS", rcp, rcm, cm)
                                elseif cm == "Defreggental"
                                        println(cm, rcp, rcm)

                                        # Discharge_present_S = run_projections_defreggental(path*"/Defreggental/", cal_S, 1981,2010, 3, "Past", "S",rcp, rcm, cm)
                                        # Discharge_future_S = run_projections_defreggental(path*"/Defreggental/", cal_S, 2071,2100,10, "Future", "S", rcp, rcm, cm)
                                        # Discharge_present_NS = run_projections_defreggental(path*"/Defreggental/", cal_NS_OP, 1981,2010, 3, "Past", "NS", rcp, rcm, cm)
                                        Discharge_future_NS = run_projections_defreggental(path*"/Defreggental/", cal_NS_OP, 2071,2100,10, "Future", "NS", rcp, rcm, cm)
                                elseif cm == "Gailtal"
                                        println(cm, rcp, rcm)

                                        # Discharge_present_S = run_projections_gailtal(path*"/Gailtal/", cal_S, 1981,2010, 3, "Past", "S",rcp, rcm, cm)
                                        # Discharge_future_S = run_projections_gailtal(path*"/Gailtal/", cal_S, 2071,2100,10, "Future", "S", rcp, rcm, cm)
                                        #Discharge_present_NS = run_projections_gailtal(path*"/Gailtal/", cal_NS_OP, 1981,2010, 3, "Past", "NS", rcp, rcm, cm)
                                        Discharge_future_NS = run_projections_gailtal(path*"/Gailtal/", cal_NS_OP, 2071,2100,10, "Future", "NS", rcp, rcm, cm)
                                elseif cm == "Palten"
                                        println(cm, rcp, rcm)
                                        if r==1 && j>=10
                                                Discharge_future_NS = run_projections_palten(path*"/Palten/", cal_NS_OP, 2071,2100,10, "Future", "NS", rcp, rcm, cm)
                                        elseif r==2
                                                Discharge_future_NS = run_projections_palten(path*"/Palten/", cal_NS_OP, 2071,2100,10, "Future", "NS", rcp, rcm, cm)
                                        end
                                        # Discharge_present_S = run_projections_palten(path*"/Palten/", cal_S, 1981,2010, 3, "Past", "S",rcp, rcm, cm)
                                        # Discharge_future_S = run_projections_palten(path*"/Palten/", cal_S, 2071,2100,10, "Future", "S", rcp, rcm, cm)
                                        #Discharge_present_NS = run_projections_palten(path*"/Palten/", cal_NS_OP, 1981,2010, 3, "Past", "NS", rcp, rcm, cm)
                                elseif cm == "Pitztal"
                                        println(cm, rcp, rcm)
                                        # Discharge_present_S = run_projections_pitztal(path*"/Pitztal/", cal_S, 1981,2010, 3, "Past", "S",rcp, rcm, cm)
                                        # Discharge_future_S = run_projections_pitztal(path*"/Pitztal/", cal_S, 2071,2100,10, "Future", "S", rcp, rcm, cm)
                                        #Discharge_present_NS = run_projections_pitztal(path*"/Pitztal/", cal_NS_OP, 1981,2010, 3, "Past", "NS", rcp, rcm, cm)
                                        Discharge_future_NS = run_projections_pitztal(path*"/Pitztal/", cal_NS_OP, 2071,2100,10, "Future", "NS", rcp, rcm, cm)
...
# path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Data/Projections/new_station_data_rcp45/rcp45/"
# # 14 different projections
# Name_Projections = readdir(path)
# for (i, name) in enumerate(Name_Projections)
#         println(i)
#         # @time begin
#         # Discharge_present = run_projections_pitztal(path*name*"/Pitztal/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal_loss_less_dates/Pitztal_Parameterfit_All_runs_best_300.csv", 1981,2010, "past_2010",3, "45")
#         # end
...
#         Discharge_present = run_projections_silbertal(path*name*"/IllSugadin/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Silbertal_Snow_Redistribution/Silbertal_Parameterfit_All_runs_snow_redistr_best_300.csv", 1981,2010, "past_2010",3)
#         Discharge_present = run_projections_silbertal(path*name*"/IllSugadin/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Silbertal_Snow_Redistribution/Silbertal_Parameterfit_All_runs_snow_redistr_best_300.csv", 2071, 2100, "future_2100", 10)
#         # Discharge_present = run_projections_paltental(path*name*"/Palten/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Palten_less_dates/Palten_Parameterfit_All_less_dates_unique_best_300.csv", 1981,2010, "past_2010",3)
#         # Discharge_present = run_projections_paltental(path*name*"/Palten/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Palten_less_dates/Palten_Parameterfit_All_less_dates_unique_best_300.csv", 2071, 2100, "future_2100", 10)
#         #Discharge_present = run_projections_gailtal(path*name*"/Gailtal/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Gailtal_less_dates/Gailtal_Parameterfit_All_less_dates_best_proj.csv", 1981,2010, "past_2010",3)
#         #Discharge_present = run_projections_gailtal(path*name*"/Gailtal/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Gailtal_less_dates/Gailtal_Parameterfit_All_less_dates_best_proj.csv", 2071, 2100, "future_2100", 10)
#         # Discharge_present = run_projections_defreggental(path*name*"/Defreggental/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Defreggental_less_dates/Defreggental_Parameterfit_All_runs_less_dates_best_proj.csv", 1981,2010, "past_2010",3)
#         # Discharge_present = run_projections_defreggental(path*name*"/Defreggental/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Defreggental_less_dates/Defreggental_Parameterfit_All_runs_less_dates_best_proj.csv", 2071, 2100, "future_2100", 10)
#         # Discharge_present = run_projections_pitztal(path*name*"/Pitztal/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal_Snow_Redistribution/Pitztal_Parameterfit_All_runs_snow_redistr_best_300.csv", 1981,2010, "past_2010",3, "85")
#         # Discharge_present = run_projections_pitztal(path*name*"/Pitztal/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal_Snow_Redistribution/Pitztal_Parameterfit_All_runs_snow_redistr_best_300.csv", 2071, 2100, "future_2100", 10, "85")
#         end
# end

# path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Data/Projections/new_station_data_rcp85/rcp85/"
# #14 different projections
# Name_Projections = readdir(path)
# for (i, name) in enumerate(Name_Projections)
#         @time begin
#         println("current scenario", name)
#         # Discharge_present = run_projections_silbertal(path*name*"/IllSugadin/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Silbertal_less_dates/Silbertal_Parameterfit_All_less_dates_best_300.csv", 1981,2010, "past_2010",3)
#         # Discharge_present = run_projections_silbertal(path*name*"/IllSugadin/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Silbertal_less_dates/Silbertal_Parameterfit_All_less_dates_best_300.csv", 2071, 2100, "future_2100", 10)
#         # Discharge_present = run_projections_paltental(path*name*"/Palten/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Palten_less_dates/Palten_Parameterfit_All_less_dates_unique_best_300.csv", 1981,2010, "past_2010",3)
#         # Discharge_present = run_projections_paltental(path*name*"/Palten/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Palten_less_dates/Palten_Parameterfit_All_less_dates_unique_best_300.csv", 2071, 2100, "future_2100", 10)
#         #Discharge_present = run_projections_gailtal(path*name*"/Gailtal/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Gailtal_less_dates/Gailtal_Parameterfit_All_less_dates_best_proj.csv", 1981,2010, "past_2010",3)
#         #Discharge_present = run_projections_gailtal(path*name*"/Gailtal/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Gailtal_less_dates/Gailtal_Parameterfit_All_less_dates_best_proj.csv", 2071, 2100, "future_2100", 10)
#         Discharge_present = run_projections_defreggental(path*name*"/Defreggental/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Defreggental_less_dates/Defreggental_Parameterfit_All_runs_less_dates_best_proj.csv", 1981,2010, "past_2010",3)
#         # Discharge_present = run_projections_defreggental(path*name*"/Defreggental/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Defreggental_less_dates/Defreggental_Parameterfit_All_runs_less_dates_best_proj.csv", 2071, 2100, "future_2100", 10)
#         #Discharge_present = run_projections_pitztal(path*name*"/Pitztal/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal_loss_less_dates/Pitztal_Parameterfit_All_runs_best_300.csv", 1981,2010, "past_2010",3, "85")
#         #Discharge_present = run_projections_pitztal(path*name*"/Pitztal/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal_loss_less_dates/Pitztal_Parameterfit_All_runs_best_300.csv", 2071, 2100, "future_2100", 10, "85")
#         end
# end
...
#for (i, name) in enumerate(Name_Projections[11:14])
         ...
         # Discharge_present = run_projections_silbertal(path*name*"/IllSugadin/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Silbertal_Snow_Redistribution/Silbertal_Parameterfit_All_runs_snow_redistr_best_300.csv", 1981,2010, "past_2010",3)
         # Discharge_present = run_projections_silbertal(path*name*"/IllSugadin/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Silbertal_Snow_Redistribution/Silbertal_Parameterfit_All_runs_snow_redistr_best_300.csv", 2071, 2100, "future_2100",10)
         #Discharge_present = run_projections_defreggental(path*name*"/Defreggental/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Defreggental_Snow_Redistribution/Defreggental_Parameterfit_All_runs_snow_redistr_best_300.csv", 1981,2010, "past_2010",3)
         #Discharge_present = run_projections_defreggental(path*name*"/Defreggental/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Defreggental_Snow_Redistribution/Defreggental_Parameterfit_All_runs_snow_redistr_best_300.csv", 2071, 2100, "future_2010",10)
         #Discharge_present = run_projections_pitztal(path*name*"/Pitztal/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal_Snow_Redistribution/Pitztal_Parameterfit_All_runs_snow_redistr_best_300.csv", 1981,2010, "past_2010",3, "45")
         #Discharge_present = run_projections_pitztal(path*name*"/Pitztal/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal_Snow_Redistribution/Pitztal_Parameterfit_All_runs_snow_redistr_best_300.csv", 2071, 2100, "future_2100",10, "45")
# #
# #
# #         Discharge_future = run_projections_silbertal(path*name*"/IllSugadin/", "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Silbertal_less_dates/Silbertal_Parameterfit_All_less_dates_best_300.csv", 2071, 2100, "future_2100", 10)
#end
# #
# end
# path = "/Volumes/Macintosh HD - Gegevens 1/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Data/Projections/new_station_data_rcp85/rcp85/"
