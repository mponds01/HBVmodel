...
"""
This function calculates the yearly root zone storage deficits (srdef) for the Pitztal
...
It incorporates a loss term to account for water that diverts away from the catchment
"""

function run_srdef_GEV_pitztal( path_to_projection, path_to_best_parameter, startyear, endyear, spinup, rcp, rcm)
        ...
        Temperature_Mean_Elevation_Min = Temperature_Elevation_Catchment_Min[:,findfirst(x-> x==2500, Elevation_Zone_Catchment_Min)]
        Temperature_Mean_Elevation_Max = Temperature_Elevation_Catchment_Max[:,findfirst(x-> x==2500, Elevation_Zone_Catchment_Max)]
        ...
        for (e, ep_method) in enumerate(EP)
                ...
                for n = 1:1:size(parameters_best_calibrations)[1]
                        ...
                        # if ploton=="yes"
                        #         Peplot = Plots.plot(title="Pitztal", titlefontsize=12)
                        ...
                        #
                        #
                        #         Pepplot = Plots.plot(title="Pitztal", titlefontsize=12)
                        ...
                        #CSV.write("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Rootzone/GEV/Pitztal_GEV_T.csv", dfgev)
...
"""
function run_srdef_GEV_pitztal_obs(path_to_best_parameter, startyear, endyear, spinup, rcp, rcm)
        local_path = "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        # 102061 im Norden
        ID_Prec_Zones = [102061, 102046]
        ...
        Temperature2 = CSV.read(local_path*"HBVModel/Pitztal/prenner_tag_17315.dat", DataFrame, header = true, skipto = 3, delim = ' ', ignorerepeated = true)
        ...
        Temperature = dropmissing(Temperature)
        Temperature2 = dropmissing(Temperature2)

        Temperature_Array = Temperature.t / 10
        Temperature2_Array = Temperature.t / 10

        Timeseries_Temp = Date.(Temperature.datum, Dates.DateFormat("yyyymmdd"))
        Timeseries_Temp2 = Date.(Temperature2.datum, Dates.DateFormat("yyyymmdd"))

        startindex = findfirst(isequal(Date(startyear, 1, 1)), Timeseries_Temp)
        endindex = findfirst(isequal(Date(2005, 12, 31)), Timeseries_Temp)
        Temperature_Daily = Temperature_Array[startindex[1]:endindex[1]]
        Dates_Temperature_Daily = Timeseries_Temp[startindex[1]:endindex[1]]

        startindex2 = findfirst(isequal(Date(2006,1,1)), Timeseries_Temp2)
        endindex2 = findfirst(isequal(Date(endyear,12,31)), Timeseries_Temp2)
        Temperature_Daily2 = Temperature2_Array[startindex2[1]:endindex2[1]]
        Dates_Temperature_Daily2 = Timeseries_Temp2[startindex2[1]:endindex2[1]]
        ...
        append!(Dates_Temperature_Daily_all, Dates_Temperature_Daily2)
        append!(Temperature_Daily_all, Temperature_Daily)
        append!(Temperature_Daily_all, Temperature_Daily2)

        #Timeseries_Temp = Timeseries[startindex[1]:endindex[1]]
        Dates_missing_Temp_all = Dates_Temperature_Daily_all[findall(x-> x == 999.9, Temperature_Daily)]
        ...
        Plots.plot(title="Pitztal", titlefontsize=12)
        for n = 1:1:size(parameters_best_calibrations)[1]
                Current_Inputs_All_Zones = deepcopy(Inputs_All_Zones)
                Current_Storages_All_Zones = deepcopy(Storages_All_Zones)
                Current_GWStorage = deepcopy(GWStorage)
...
function run_srdef_GEV_pitztal_withplot( path_to_projection, path_to_best_parameter, startyear, endyear, period, spinup, ploton, rcp, rcm)
        local_path = "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        path_to_folder = "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Results/Rootzone/Pitztal/"*rcp*"/"*rcm*"/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        ID_Prec_Zones = [102061, 102046]
...
function run_srdef_GEV_pitztal_obs_withplot(path_to_best_parameter, startyear, endyear, period, spinup, ploton, rcp, rcm)
        local_path = "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/"
        # ------------ CATCHMENT SPECIFIC INPUTS----------------
        # 102061 im Norden
        ID_Prec_Zones = [102061, 102046]
        ...
        for n = 1:1:size(parameters_best_calibrations)[1]
                ...
                if ploton=="yes"
                        ...
                        srdefmaxyear = Plots.plot(title="Pitztal", titlefontsize=12)
                        ...
                        srdeftimeser=Plots.plot(title="Pitztal", titlefontsize=12)
                        ...
                        srdeftime=Plots.plot(title="Pitztal", titlefontsize=12)
                        ...
                        srdefcum = Plots.plot(title="Pitztal", titlefontsize=12)
                        ...
                        cumzoom = Plots.plot(title="Pitztal", titlefontsize=12)
                        ...
                        alltimeser = Plots.plot(title="Pitztal", titlefontsize=12)
                        ...
                        Srmax_grass = Float64[]
                        parameters = Plots.plot(title="Pitztal", titlefontsize=12)
                        for n = 1:1:size(parameters_best_calibrations)[1]
                                beta_Bare, beta_Forest, beta_Grass, beta_Rip, Ce, Interceptioncapacity_Forest, Interceptioncapacity_Grass, Interceptioncapacity_Rip, Kf_Rip, Kf, Ks, Meltfactor, Mm, Ratio_Pref, Ratio_Riparian, Soilstoaragecapacity_Bare, Soilstoaragecapacity_Forest, Soilstoaragecapacity_Grass, Soilstoaragecapacity_Rip, Temp_Thresh = parameters_best_calibrations[n, :]
        ...
        #Recurranceinterval
        if ploton =="yes"
                gev = Plots.plot(title="Pitztal", titlefontsize=12)
                scatter!(xt,yt)
                xaxis!("xti")
                yaxis!("yti")
                ...
                gev2  = Plots.plot(title="Pitztal", titlefontsize=12)
...
function run_srmax_rcps_pitztal()
        path_to_best_parameter= "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal/Best/Parameterfit_less_dates_snow_redistr_1M_best_300.csv"
        ...
        #rcms = readdir(local_path*rcp)
                ...
                for (j,rcm) in enumerate(rcms)
                        ...
                        run_srdef_GEV_pitztal(path_to_projection, path_to_best_parameter, 2071,2100, 3,  rcp, rcm)
                        #run_srdef_GEV_pitztal(path_to_projection, path_to_best_parameter, 1978,2010,3,  rcp, rcm)
                        run_srdef_GEV_pitztal(path_to_projection, path_to_best_parameter, 1981,2013, 3,  rcp, rcm)
                        run_srdef_GEV_pitztal_obs(path_to_best_parameter, 1983,2013, 3,  rcp, rcm)
...
function GEVresult_pitztal(path_to_best_parameter, catchment_name, rcp, rcm)
        ...
        for (e,ep_method) in enumerate(PE)
                violin!(-obs_past[:,2*e+1], color=colour[e], label=false)
...
#run_srdef_GEV_pitztal("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Data/Projections/rcp45/CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day/Pitztal/", "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal/Best/Parameterfit_less_dates_snow_redistr_best_100.csv", 2071,2100, 3,  "rcp45", "CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day")
# run_srdef_GEV_pitztal("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Data/Projections/rcp45/CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day/Pitztal/", "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal/Best/Parameterfit_less_dates_snow_redistr_best_100.csv", 1978,2010, 3,"rcp45", "CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day" )
# run_srdef_GEV_pitztal("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Data/Projections/rcp45/CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day/Pitztal/", "/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal/Best/Parameterfit_less_dates_snow_redistr_best_100.csv", 1981,2013, 3, "rcp45", "CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day" )
#run_srdef_GEV_pitztal_obs("/Users/magali/Documents/1. Master/1.4 Thesis/02 Execution/01 Model Sarah/Calibrations/Pitztal/Best/Parameterfit_less_dates_snow_redistr_1M_best_300.csv", 1983,2013, 3,  "rcp45", "CNRM-CERFACS-CNRM-CM5_rcp45_r1i1p1_CLMcom-CCLM4-8-17_v1_day")
#
